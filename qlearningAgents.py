...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        actions = self.getLegalActions(state)
        if not actions:  # Check if the list of actions is empty
            return 0
    ...
    def update(self, state, action, nextState, reward):
        ...
        """
        "*** YOUR CODE HERE ***"
        self.q_vals[(state, action)] = (1 - self.alpha) * self.getQValue(state, action) + self.alpha * (
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        q_value = sum(self.weights[feature] * featureVector[feature] for feature in featureVector)
        return q_value
